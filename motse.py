...
from metrics import cosine_similarity,spearman,pearson_matrix,pearson
...
class MoTSE(object):
    ...
    def _prepare_batch_data(self, batch_data):
        smiles, inputs, node_feats, kpgt_feats = batch_data
        inputs.ndata['node_pka'] = inputs.ndata['node_pka']
        kpgt_feats = kpgt_feats

        return smiles, inputs, kpgt_feats

    def extract_knowledge(self, task, model_path, probe_data):
        smiles, gs, kpgt_feats = self._prepare_batch_data(probe_data['data'])
        model = load_model(self.device, source_model_path=model_path)

        node_feats = gs.ndata['node_pka']
        gs.ndata['node_pka'] = Variable(node_feats, requires_grad=True)
        g_reps, predictions = model(gs, kpgt_feats)
        predictions.mean().backward()
        gradient_mul_input = gs.ndata['node_pka'].grad * gs.ndata['node_pka']
    ...
    def cal_sim(self, source_tasks, target_tasks,
        ...
        for i, target_task in enumerate(target_tasks):
            ...
            pd.DataFrame(results_dict).to_csv(self.path + f'{filename}.csv',
