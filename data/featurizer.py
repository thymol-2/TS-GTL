...
def featurize_molecule(mol):
    ...
    atom_feats = atom_feats_dict['h'] # 获取特征张量
    edge_feats = bond_featurizer(mol)

    return atom_feats, edge_feats


def get_pKa_features(smiles):
    ...
    pka_features = torch.zeros(num_atoms, 1, dtype=torch.float32)

    # 将具有 pKa 值的原子的索引位置设置为相应的 pKa 值
    for state in protonation_states:
        pka_features[state.reaction_center_idx] = state.pka
...
def smiles_to_graph(smiles, vocab, max_length=5, n_virtual_nodes=8, add_self_loop=True):
    ...
    atom_features = []
    pka_features = get_pKa_features(smiles)

    for atom_id in range(n_atoms):
        atom = mol.GetAtomWithIdx(atom_id)
        atom_feats = atom_featurizer_all(atom)
        # atom_features.append(atom_featurizer_all(atom))

        pka_list = pka_features.tolist()
        atom_feats.append(pka_list[atom_id][0])
    ...
    for atom_id in range(n_atoms):
        if atom_id not in bonded_atoms:
            ...
            # node_features.append(VIRTUAL_ATOM_INDICATOR)
    ...
    # node_feats = torch.cat([atom_features, pka_features], dim=1)
    # node_features = convert_features(node_features)

    num_nodes = g.num_nodes()
    complete_features = torch.zeros((num_nodes-n_atoms, 138), dtype=torch.float32)

    atom_features = torch.tensor(atom_features)
    # 填充剩余虚拟节点
    # 这里保持零值
    node_feats = torch.cat([atom_features, complete_features], dim=0)
    g.ndata['node_pka'] = node_feats
